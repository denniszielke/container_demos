...
az network firewall network-rule create --firewall-name $FW_NAME --collection-name "hcp" --destination-addresses "*" --destination-ports "1194" --name "allow master tags" --protocols "Any" --resource-group $KUBE_GROUP --source-addresses "*" --action "Allow" --description "allow tunnel access" --priority 120
