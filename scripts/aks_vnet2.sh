...
DEPLOYMENT_NAME="dzdet26" # here enter unique deployment name (ideally short and with letters for global uniqueness)
...
AAD_GROUP_ID="c2e1d370-a29b-472b-b786-887df2874cc2 --enable-azure-rbac" 
...
echo "deploying version $KUBE_VERSION"
...
if [ "$USE_PRIVATE_API" == "true" ]; then
    ACTIVATE_PRIVATE_LINK=" --enable-private-cluster --disable-public-fqdn" # --enable-apiserver-vnet-integration --apiserver-subnet-id $KUBE_API_SUBNET_ID " # --disable-public-fqdn
else
...
az aks update --disable-defender --resource-group $KUBE_GROUP --name $KUBE_NAME
